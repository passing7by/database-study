...
-- ② 여러 테이블을 결합하여 하나의 결과를 반환하는 것 -- JOIN
-- ③ 특정 조건을 기준으로 데이터를 필터링하는 명령 -- WHERE
...
-- ① 서브쿼리는 데이터를 결합하고, JOIN은 데이터를 필터링한다. -- JOIN, WHERE
...
-- employees(id, name)
-- 서브쿼리: 각 직원의 참여 중인 프로젝트 수
SELECT COUNT(*)
FROM employee_projects
WHERE employee_id = 1;
-- 메인쿼리
SELECT DISTINCT e.id AS 직원ID, e.name AS 직원명, (
	SELECT COUNT(*)
	FROM employee_projects
	WHERE employee_id = e.id -- 해당 직원의 참여 프로젝트 수만 셈
) AS '참여 중인 프로젝트 수'
FROM employees e 
...
-- 정답: 
-- departments(id, name)
-- employees(department_id, id)
-- | 부서 | 직원 | 테이블을 만들고, select절에서 직원 수를 구함
-- 서브쿼리
SELECT d.id, d.name, e.id, e.name
FROM departments d 
JOIN employees e ON d.id = e.department_id;
-- 메인쿼리
SELECT 부서ID, 부서명, COUNT(직원ID) AS '직원 수'
FROM (
	SELECT d.id AS 부서ID, d.name AS 부서명, e.id AS 직원ID, e.name AS 직원명
	FROM departments d 
	JOIN employees e ON d.id = e.department_id
) AS sub
GROUP BY 부서ID, 부서명;
...
-- 정답: 
-- employees(id, name)
-- salary_records(employee_id, amount)
-- 직원id별 받은 급여 합계 테이블 과 employees 테이블 JOIN
-- 서브쿼리: 직원id별 받은 급여 합계
SELECT employee_id, SUM(amount)
FROM salary_records
GROUP BY employee_id;
-- 메인쿼리
SELECT id, name, 총지급급여
FROM employees e 
JOIN (
	SELECT employee_id , SUM(amount) AS 총지급급여
	FROM salary_records
	GROUP BY employee_id
) AS sub
...
-- 그룹으로 묶은 다음 HAVING절에서 평균 급여 이상인 조건으로 필터링
-- 서브쿼리
SELECT AVG(salary)
FROM employees;
-- 메인 쿼리
...
-- employee_projects(employee_id, project_id)
-- 풀이 1)
-- 서브쿼리-JOIN: 프로젝트당 참여 직원 수
SELECT project_id, COUNT(employee_id)
FROM employee_projects
GROUP BY project_id;
-- 메인쿼리
SELECT p.id, p.name, 참여_직원수
FROM projects p
JOIN (
	SELECT project_id, COUNT(employee_id) AS 참여_직원수
	FROM employee_projects
	GROUP BY project_id
) AS sub
	ON p.id = sub.project_id
GROUP BY p.id, p.name
ORDER BY 참여_직원수 DESC
LIMIT 1;

-- 풀이 2)
-- 서브쿼리-FROM: 프로젝트당 참여 직원 수와 프로젝트명
SELECT p.id, p.name, COUNT(ep.employee_id)
FROM employee_projects ep
JOIN projects p 
	ON ep.project_id = p.id
GROUP BY p.id, p.name;
-- 메인쿼리
SELECT 프로젝트ID, 프로젝트명, 참여인원
FROM (
	SELECT p.id AS 프로젝트ID, p.name AS 프로젝트명, COUNT(ep.employee_id) AS 참여인원
	FROM employee_projects ep
	JOIN projects p 
		ON ep.project_id = p.id
	GROUP BY p.id, p.name
