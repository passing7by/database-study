...
-- 정답: 4
-- UNION(중복 레코드(튜플)를 제거하고 합침) 연산자를 사용했을 때와 같음
...
SELECT name
FROM library_staff
WHERE name IN (SELECT name FROM members);
SELECT name
FROM members
WHERE name IN (SELECT name FROM library_staff);
-- 문제 6
