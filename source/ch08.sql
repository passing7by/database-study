...
-- 예: 전체 학생의 평균 키도 의미가 있으나, 성별로 나누어 평균 키를 구하면 조금 더 유의미한 정보를 얻음
...
SELECT 
  ...
  SUM(amount) AS '총 매출' -- 3) 특정 도시의 연별별 총 매출 집계 조회
FROM sales -- 1) sales 테이블을
GROUP BY city, YEAR(sale_date); -- 2) 도시와 연도로 그룹화한 후

-- 참고로 실무에서는 별칭 이렇게 안 지음... 띄어쓰기 없이 영어로 지음 예)total_count

-- 3) SELECT 절에 올 수 있는 컬럼이 제한적이다.
-- 사용 가능한 컬럼:
-- - 그룹화 컬럼: GROUP BY 절에서 지정한 컬럼(그룹을 지을 때 사용한 컬럼)
...
ORDER BY amount DESC; -- 3) amount 기준으로 내림하순 정렬함

-- 다중 컬럼 정렬: 결제 테이블에서 결제 유형은 오름차순, 결제 금액는 내림차순으로 정렬하려면?
-- 
...
-- LIMIT 절을 이용해 반환하여는 레코드의 개수를 정의
...
-- LIMIT 절에 OFFSET 키워를를 추가해 읽어 올 데이터의 시작 지점을 조정할 수 있음
...
-- 확인
SHOW DATABASES; -- DB들 확인
SELECT DATABASE();
SHOW TABLES; -- market DB 안에 있는 테이블들 확인
...
SELECT 
  u.nickname AS '주문자명', 
  SUM(p.amount) AS '주문 총액'
FROM users u
JOIN orders o ON u.id = o.user_id
JOIN payments p ON o.id = p.order_id
GROUP BY u.id;
-- 주문 총액이 30,000원 이상인 사용자만
SELECT
  u.nickname AS '주문자명', 
...
-- 상품별(product.id) 판매 수량(order_details.count & orders.status = '배송 완료')
-- 테이블간 JOIN 결과 확인
SELECT p.id, p.name, od.count, o.id
FROM products p
JOIN order_details od ON p.id = od.product_id
JOIN orders o ON od.order_id = o.id;
-- p.id별 od.count의 합 확인
SELECT p.id, p.name, SUM(od.count)
FROM products p
JOIN order_details od ON p.id = od.product_id
JOIN orders o ON od.order_id = o.id
GROUP BY p.id;
-- od.count의 합이 큰 순으로 정렬
SELECT p.id, p.name, SUM(od.count)
FROM products p
JOIN order_details od ON p.id = od.product_id
JOIN orders o ON od.order_id = o.id
...
GROUP BY p.id
ORDER BY SUM(od.count) DESC
LIMIT 3; -- 3위, 4위가 동률일 때, DB엔진 내부 구현에 따라 임의로 가져와짐
-- SUM()이 동률일 때의 정렬 기준을 추가로 설정해 줌
SELECT 
  p.name AS '상품명', 
  SUM(od.count) AS '판매수량'
FROM products p
...
-- 정답: 
-- order_details.count * products.price
-- 상품 가격을 기준으로 한 이론상 매출액: 실제 결제와 무관하며, 할인/부분결제/결제 누락 등을 반영하지 않음
