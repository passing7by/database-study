-- Active: 1749604230285@@127.0.0.1@3306@relation
/*
6. 관계 만들기
6.1 관계란
*/
...
CREATE TABLE products (
  ...
  dc_rate INTEGER CHECK (dc_rate <= 50 AND dc_rate >= 0),      -- 할인율(0~50% 제한)
...
CREATE TABLE players (
  ...
  -- teams 테이블의 id 컬럼을 가리킴 => 선수가 어느 팀에 소속되어 있는지르 연결(관계 형성)
);
...
CREATE TABLE passport (
  ...
  person_id INTEGER UNIQUE, -- 일대일 관계인 경우, 외래키에 UNIQUE 제약 조건이 있어야 함
  PRIMARY KEY (id),
...
INSERT INTO stargram.users (nickname, email)
VALUES
...
-- 데이터 조회
SELECT * FROM stargram.users;

-- 2. 사진 테이블 만들기
CREATE TABLE stargram.photos (
  id INTEGER AUTO_INCREMENT, -- 아이디(자동으로 1씩 증가)
...
-- photos 데이터 삽입
INSERT INTO stargram.photos (filename, user_id)
VALUES
	-- 1번 사용자가 게시한 사진
	('길고양이.jpg', 1),
	('일몰.jpg', 1),
...
-- 데이터 조회
SELECT *
FROM stargram.photos;

-- 3. 댓글 테이블 만들기
CREATE TABLE stargram.comments (
  id INTEGER AUTO_INCREMENT, -- 아이디(자동으로 1씩 증가)
...
INSERT INTO stargram.comments (body, user_id, photo_id)
VALUES
	-- 1번 사진에 달린 댓글
	('미야옹~', 1, 1),
	('냐옹!', 2, 1),
...
-- 데이터 조회
SELECT *
FROM stargram.comments;

-- 4. 개인 설정 테이블 만들기
CREATE TABLE stargram.settings (
  id INTEGER AUTO_INCREMENT, -- 아이디(자동으로 1씩 증가)
  private BOOLEAN NOT NULL, -- 계정 공개 여부(NULL 허용 안 함)
  account_suggestions BOOLEAN NOT NULL, -- 계정 추천 여부(NULL 허용하지 않음)
  user_id INTEGER UNIQUE, -- 사용자 아이디(고유한 값만 허용), 1:1 연결을 위해 UNIQUE wlwjd
  PRIMARY KEY (id), -- 기본키 지정: id
  FOREIGN KEY (user_id) REFERENCES users(id) -- 외래키 지정: user_id
);

-- settings 데이터 삽입
INSERT INTO stargram.settings (private, account_suggestions, user_id)
VALUES
...
-- 데이터 조회
SELECT *
FROM stargram.settings;

-- 5. 좋아요 테이블 만들기
CREATE TABLE stargram.likes (
  id INTEGER AUTO_INCREMENT, -- 아이디(자동으로 1씩 증가)
...
-- likes 데이터 삽입
INSERT INTO stargram.likes (user_id, photo_id)
VALUES
