...
-- 6 Sales 부서의 최대 연봉과 최소 연봉차이를 구하세요.
select max(salary) - min(salary) as '최대 연봉과 최소 연봉 간 차이 (부서: Sales)' from employees
where department = 'Sales';
-- 7 가장 높은 연봉을 받는 직원은 전체 평균 연봉 대비 얼마를 더 받는지 구하세요.
select max(salary) - avg(salary) as '최대 연봉과 평균 연봉 간 차이' from employees;