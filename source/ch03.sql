-- Active: 1749604230285@@127.0.0.1@3306@mapdonalds
/*
3. 데이터 필터링하기
3.1 데이터 필터링이란
*/
...
-- students 데이터 삽입
INSERT INTO students (id, nickname, math, english, prigramming)
VALUES
	(1, 'Sparkles', 98, 96, 93),
	(2, 'Soldier', 82, 66, 98),
...
SELECT * FROM students
WHERE math >= 90 AND english >= 90 AND prigramming >= 90;

-- 2. 75점 미만이 하나라도 있는 학생은?
SELECT * FROM students
WHERE math < 75 OR english < 75 OR prigramming < 75;

-- 3. 모든 학생의 총점은?
SELECT math + english + prigramming AS 총점
FROM  students;

-- 4. 모든 학생의 평균은?
SELECT (math + english + prigramming)/3 AS 평균
FROM  students;

-- 5. 총점이 270 이상인 학생의 닉네임, 총점, 평균은?
SELECT nickname, math + english + prigramming AS 총점, (math + english + prigramming)/3 AS 평균
FROM students
WHERE math + english + prigramming >= 270;
SELECT *
FROM (SELECT nickname, (math + english + prigramming) AS 총점, (math + english + prigramming)/3 AS 평균 FROM students) AS t
WHERE 총점 >= 270;
