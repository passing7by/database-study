...
-- 여러 행 또는 여러 컬럼을 반환하면 SQL이 어떤 값을 선택해야할지 몰라 에러 발생
...
-- 의미: 주문과 결제 테이블에서 COCOA PAY를 사용한 사용자가 있다면 그 사람을 제외한 사용자를 뽑아줘
